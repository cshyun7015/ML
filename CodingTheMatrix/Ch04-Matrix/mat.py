...
def equal(A, B):
    ...
    """
    assert A.D == B.D
    A_key = [key for key, value in A.f.items() if value != 0]
    B_key = [key for key, value in B.f.items() if value != 0]
    
    if (len(A_key) == 0 and len(B_key) == 0): return True
    if (set(A_key) != set(B_key)): return False
    return True if (sum([getitem(A, key) != getitem(B, key) for key, value in A.f.items()]) == 0) else False
