...
def equal(u,v):
    ...
    """
    assert u.D == v.D
    u_key = [key for key, value in u.f.items() if value != 0]
    v_key = [key for key, value in v.f.items() if value != 0]
    
    if (len(u_key) == 0 and len(v_key) == 0): return True
    if (set(u_key) != set(v_key)): return False
    return True if (sum([getitem(u, d) != getitem(v, d) for d in u.D]) == 0) else False
