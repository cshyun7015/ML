...
def orthonormalize(L):
    ...
    '''
    vstarlist = orthogonalize(L)
    norms = [math.sqrt(v*v) for v in vstarlist]
    return [1/norms[i]*vstarlist[i] for i in range(len(norms))]
...
def aug_orthonormalize(L):
    ...
    norms = [math.sqrt(v*v) for v in vstarlist]
    Rlist = [adjust(sigma_vecs[i], norms) for i in range(len(sigma_vecs))]
    return Qlist, Rlist
